...
class MCTSSearcher:
    ...
    def compute_reward(self, generated_tokens, problem: dict, prefix_len: int):
        """
        Calculates reward using the robust, two-step Yosys + Standalone ABC workflow.
        1. Verifies functionality with Icarus Verilog.
        2. Synthesizes to a BLIF file using Yosys.
        3. Maps and analyzes for PPA using a separate, standalone ABC executable.
        4. Parses the text output from ABC to get area and delay.
        """
        # Create a fresh workspace for this evaluation
        ws = os.path.join(self.workspace_base, str(uuid.uuid4()))
        os.makedirs(ws, exist_ok=True)
        dut_path = os.path.join(ws, 'dut.v')

        # Decode the generated Verilog code
        ...
        # Write the Verilog code to dut.v
